...
internal static class Patch_GenerateSenatorByPawnkind
{
    public static bool Prefix(ref Pawn __result, WorldComponent_Senators __instance, Faction faction)
    {
        ...
        rules.pawnKindPerFaction.TryGetValue(faction.def.defName, out var pawnKindDefName);
        if(string.IsNullOrEmpty(pawnKindDefName))
            senatorPawnKindDef = VFEC_DefOf.VFEC_RepublicSenator;
        else
            senatorPawnKindDef = DefDatabase<PawnKindDef>.GetNamedSilentFail(pawnKindDefName) ?? VFEC_DefOf.VFEC_RepublicSenator;
