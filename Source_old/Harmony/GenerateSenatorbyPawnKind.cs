...
namespace InsectRepublic
{
    ...
    internal static class GenerateSenatorByPawnKind
    {
        public static bool Prefix(ref Pawn __result, WorldComponent_Senators __instance, Faction faction)
        {
            if (faction?.def is null || __instance?.world?.worldPawns is null)
                return true;
            var senatorPawnKindDef = faction.def.GetModExtension<FactionExtension_SenatorPawnKind>()?.pawnKindDef ?? VFEC_DefOf.VFEC_RepublicSenator;
